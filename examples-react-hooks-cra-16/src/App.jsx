...
const notify = (notificationn) => toast.success(`New Notification: ${notification.title}`);

function App() {
  const { notifications, loading, loadMore, refresh, hasMore } = useFeed();
  const [panelView, setPanelView] = useState('preferences');
    ...
    <>
      ...
      <div className="container">
        ...
        >
          ...
          <button className="p-2 hover:bg-slate-200 ml-3" onClick={markAllAsRead}>
            markAllAsRead
          </button>
          <button className="p-2 hover:bg-slate-200 ml-3" onClick={deleteAllNotifications}>
        ...
        <nav className="mb-2 border border-t-0 border-l-0 border-r-0 border-gray-200">
          <ul className="flex flex-row items-center gap-5 p-3 ">
            ...
            <li>
              ...
              <select
                className="p-1 mr-5 rounded outline-gray-400 bg-slate-200"
                defaultValue={'hemanditwiz@gmail.com'}
                onChange={(event) => {
                  setUserId(event.target.value);
                }}
              >
                <option value={'hemanditwiz@gmail.com'}>User 1</option>
                <option value={'anandrmedia@gmail.com'}>User 2</option>
              </select>
            </li>
            <li>
              <Popper
                renderButton={(open) => {
                  return (
                    <>
                      <div className="flex">
                        <Notification />
                        {unreadCount && <span className="text-red-500 text-md">•</span>}
                      </div>
                      <ChevronDownIcon
                      ...
                      {panelView === 'preferences' ? (
                        <div className="flex flex-col gap-4">
                          {preferences?.categories?.map((categories) => (
                            ...
                            >
                              <div className="uppercase font-semibold mb-3">{categories.name}</div>
